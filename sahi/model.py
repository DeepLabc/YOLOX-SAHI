...
class YoloXDetectionModel(DetectionModel):
    ...
    def perform_batch_inference(self, init_image, batch_image, slice_size: int = None, slice_box=None):
        try:
            import yolox
        except ImportError:
            raise ImportError('Please run "pip install -U yolox" ' "to install YOLOX first for YOLOX inference.")
        ...
        # Confirm model is loaded
        assert self.model is not None, "Model is not loaded, load it by calling .load_model()"
        tensor_img = batch_image.float()
        tensor_img = tensor_img.cuda()

        with torch.no_grad():
            prediction_result = self.model(tensor_img)  #[batch, 8400, 85]
            prediction_result = postprocess(
                    prediction_result, len(self.classes), self.confidence_threshold,
                    self.nms_threshold, class_agnostic=True
                ) # list([target_num, 7]*batch)
        ...
        if (len(prediction_result) != 0):
            for i in range(len(prediction_result)):
                ...
                # covert to original predict box
                ...
                for b_id in range(len(bboxes)):
                    ...
                    object_prediction = ObjectPrediction(
                        ...
                        shift_amount=top_left,
                        score=score,
                        full_shape=[init_image.shape[0], init_image.shape[1]],
                    )
