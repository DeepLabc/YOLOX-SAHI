...
from mySahi.slicing import slice_image_batch
...
def get_sliced_prediction(
    ...
    for group_ind in range(num_group):
        ...
        for object_prediction in prediction_result.object_prediction_list:
            if object_prediction:  # if not empty
                #print(object_prediction.get_shifted_object_prediction())
    ...
    #         postprocess=None,
    #     )
    #     # print(prediction_result.object_prediction_list)
...
def get_batch_prediction(
    ...
    init_image, slice_image_result, slice_box, slice_size= slice_image_batch(
        image=image,
        slice_height=slice_height,
        slice_width=slice_width,
        overlap_height_ratio=overlap_height_ratio,
    ...
    if verbose == 1 or verbose == 2:
        tqdm.write(f"Performing prediction on {len(slice_box)} number of slices.")

    time_start = time.time()
    detection_model.perform_batch_inference(init_image, slice_image_result, slice_size, slice_box)
